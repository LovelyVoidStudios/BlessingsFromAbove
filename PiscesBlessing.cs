...
{
    public class PiscesBlessing : GroupAccessory
    {
    ...
    public class PiscesBlessingPlayer : ModPlayer
    {
        public bool PiscesBlessing = false;

        public override void ResetEffects()
        {
            PiscesBlessing = false;
        }

        public override void PostUpdateRunSpeeds()
        {
            // We only want our additional changes to apply if ExampleStatBonusAccessory is equipped and not on a mount.
            if (Player.mount.Active || !PiscesBlessing)
            {
                return;
            }
